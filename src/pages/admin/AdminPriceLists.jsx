...
const LIST_COLORS = ['bg-blue-600', 'bg-emerald-600', 'bg-purple-600', 'bg-amber-600'];
...
export default function AdminPriceLists() {
  ...
  function downloadTemplate() {
    ...
    link.href = url;
    link.download = 'plantilla-lista-precios-daval.csv';
    link.click();
  ...
  const canSave = form.name && form.fileName && (form.scope === 'all' || form.clientIds.length > 0);

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-2xl font-bold text-gray-100">Listas de Precios</h2>
          <p className="text-sm text-gray-400 mt-1">Crea listas por archivo y asigna su cobertura comercial</p>
        </div>
        <button
          onClick={openCreate}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {priceLists.map((list, idx) => {
          ...
          const exactPriceCount = list.itemCount || Object.keys(list.pricesBySku || {}).length;
          return (
            <div key={list.id} className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 overflow-hidden">
              <div className={`h-2 ${LIST_COLORS[idx % LIST_COLORS.length]}`} />
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${LIST_COLORS[idx % LIST_COLORS.length]} flex-shrink-0`}>
                      <ListOrdered className="w-5 h-5 text-white" />
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-bold text-gray-100 text-lg truncate">{list.name}</h3>
                      <p className="text-xs text-gray-400">{getScopeLabel(list)}</p>
                    </div>
                  </div>
                  <button onClick={() => openEdit(list)} className="p-1.5 text-gray-500 hover:text-blue-400 hover:bg-blue-950 rounded-lg transition">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center py-2 border-b border-gray-700">
                    ...
                    <span className="text-sm font-bold text-blue-400">{list.scope === 'all' ? clients.length : assignedIds.length}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-gray-700">
                  ...
                  <div className="flex justify-between items-center py-2">
                    ...
                    <span className="text-sm font-semibold text-gray-200 truncate max-w-[160px]">{list.fileName || 'Sin archivo'}</span>
                  </div>
                </div>
      ...
      {showModal && (
        <Modal title={editList ? `Editar ${editList.name}` : 'Nueva lista de precios'} onClose={() => setShowModal(false)}>
          <div className="space-y-5">
            <div>
              ...
              <input
                ...
                placeholder="Ej. Lista Ferreterias Mayoristas"
              />
            </div>

            <div>
              <label className={labelClass}>Aplicacion de la lista *</label>
